...
function esq_sectionsort_section_js() {
    echo <<<EOB
        ...
        <script src="index.php?event=plugin_prefs.esq_sectionsort&step=get_js&js=sortable"></script>
...
function esq_sectionsort_setup($event, $step) {
    switch ($step) {
        case 'installed':
            if (esq_sectionsort_checkDB() && !esq_sectionsort_checkDB_update()) {
...
function esq_sectionsort_checkDB_update() {
    ...
    return isset($columns['categorysort']);
}

function esq_sectionsort_setupDB($lifecycle) {
    if (
        safe_alter('txp_section', 'ADD `sectionsort` VARCHAR(128) NULL') &&
        safe_alter('txp_category', 'ADD `categorysort` VARCHAR(128) NULL')
    ) {
...
function esq_sectionsort_setupDB_update($lifecycle) {
    if (
        safe_alter('txp_section', 'MODIFY `sectionsort` VARCHAR(128) NULL') &&
        safe_alter('txp_category', 'ADD `categorysort` VARCHAR(128) NULL')
    ) {
        return '<strong>esq_sectionsort</strong> and DB updated OK.';
    } else {
        return array(
            '<strong>esq_sectionsort</strong> updated, but DB update failed.',
            E_ERROR
        );
    }
}
...
function esq_sectionsort_revertDB() {
    if (
        safe_alter('txp_section', 'DROP `sectionsort`') &&
        safe_alter('txp_category', 'DROP `categorysort`')
    ) {
                    ...
                    $(function() {
                        ...
                        $('#section_container table').css('table-layout', 'fixed').find('tbody tr').prepend('<td></td>').not('#txp_section_default').appendTo('#section_container tbody').sortElements(function(a, b) {
                            ...
                            stop: function() {
                                ...
                                $.ajax(
                                    'index.php?event=plugin_prefs.esq_sectionsort&step=put&type=section',
                                    {
                                        type: 'POST',
                                        data: sectionsort,
                                        dataType: 'json',
                        ...
                        }).find('tr').not(':first-child').find('td:first-child').html('&uarr;&darr;');
                    });
EOB;
            }
        break;
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                echo <<<EOB
                    $(function() {
                        ...
                        $('.categorysort > .categorysort').parent().each(function() {
                            $(this).sortable({
                                items: '> .categorysort',
                                handle: '.handle',
                                stop: function() {
                                    ...
                                    $.ajax(
                                        'index.php?event=plugin_prefs.esq_sectionsort&step=put&type=category',
                                        {
                                            type: 'POST',
                                            data: categorysort,
                                            dataType: 'json',
                            ...
                            }).find('> .categorysort').sortElements(function(a, b) {
